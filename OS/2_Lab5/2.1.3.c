...
if (is_empty(q)) return (Process){-1};
...
int time_slice = (p.remaining_time < quantum[q_index]) ? p.remaining_time : quantum[q_index];
...
int original_index = p.pid - 1;
processes[original_index].completion_time = p.completion_time;
processes[original_index].waiting_time = p.waiting_time;
...
printf("\nPID\tArrival\tBurst\tWaiting\tCompletion\tTurnaround\n");
for (int i = 0; i < n; i++) {
